...
export default function Shell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    setMounted(true);
    setLocalRole(typeof window !== 'undefined' ? localStorage.getItem('impersonated_role') as UserRole : null);
  }, [pathname, router]);

  if (pathname === '/') return <>{children}</>;
  if (status === "loading") return <div className="min-h-screen bg-background animate-pulse" />;
  if (!session && pathname !== '/') {
    router.push('/');
  ...
  const role = (sessionRole === 'super_admin' ? localRole : sessionRole) as UserRole;
  ...
  return (
    <SidebarProvider defaultOpen={true}>
        <Sidebar collapsible="icon" className="peer border-r border-border/40 glass shadow-xl z-50">
        ...
        <SidebarInset className="bg-background flex-1 flex flex-col w-full min-w-0 overflow-hidden transition-all duration-200 ease-linear md:peer-data-[state=expanded]:ml-64 md:peer-data-[state=collapsed]:ml-12">
                ...
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="flex items-center gap-3 p-1.5 hover:bg-primary/5 rounded-xl h-12">
                    <Avatar className="h-9 w-9 rounded-xl shadow-md overflow-hidden border border-border/50">
                      <AvatarImage src={session?.user?.image || `https://picsum.photos/seed/${role}/100/100`} />
