          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="max-w-[95vw] sm:max-w-[500px] rounded-xl border-none glass p-0 overflow-hidden shadow-2xl">
              <DialogHeader className="p-5 bg-primary/5 border-b border-primary/5 text-left">
                <DialogTitle className="font-headline text-lg font-black tracking-tight">Register Asset</DialogTitle>
              </DialogHeader>
        ...
        <CardHeader className="p-4 sm:p-5 border-b border-primary/5 bg-primary/[0.02]">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56 glass rounded-xl p-2 border-none">
                <DropdownMenuLabel className="text-[10px] uppercase font-black tracking-widest opacity-40 px-2 py-1.5">Availability</DropdownMenuLabel>
                <DropdownMenuCheckboxItem checked={statusFilters.includes('In Stock')} onCheckedChange={() => toggleStatusFilter('In Stock')} className="rounded-lg font-bold text-sm py-2"><CheckCircle2 className="mr-3 h-4 w-4 text-emerald-500" /> In Stock</DropdownMenuCheckboxItem>
                <DropdownMenuCheckboxItem checked={statusFilters.includes('Low')} onCheckedChange={() => toggleStatusFilter('Low')} className="rounded-lg font-bold text-sm py-2"><Clock className="mr-3 h-4 w-4 text-amber-500" /> Low Stock</DropdownMenuCheckboxItem>
                <DropdownMenuCheckboxItem checked={statusFilters.includes('Out of Stock')} onCheckedChange={() => toggleStatusFilter('Out of Stock')} className="rounded-lg font-bold text-sm py-2"><XCircle className="mr-3 h-4 w-4 text-destructive" /> Out of Stock</DropdownMenuCheckboxItem>
                {statusFilters.length > 0 && <><DropdownMenuSeparator /><DropdownMenuItem onClick={() => setStatusFilters([])} className="rounded-lg font-black text-[10px] uppercase tracking-widest justify-center text-primary py-2.5">Reset Filters</DropdownMenuItem></>}
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </CardHeader>
            ...
            <Table>
              <TableHeader className="bg-primary/[0.01]">
                <TableRow className="border-none">
                  <TableHead className="py-4 pl-8 font-black uppercase text-[10px] tracking-widest">SKU ID</TableHead>
                  ...
                  <TableRow key={p.id} className="border-none hover:bg-primary/[0.03] transition-colors group">
                    ...
                    <TableCell>
                      <div className="flex flex-col leading-tight">
                        ...
                        {p.mrp > p.price && <span className="text-[10px] text-muted-foreground font-black line-through opacity-50">₹{p.mrp.toLocaleString()}</span>}
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell><Badge className="bg-primary/10 text-primary border-none font-black text-[10px] px-2 py-0.5 rounded-md">{p.gst}</Badge></TableCell>
                    <TableCell>
                      <Badge className={cn(
                        "rounded-lg px-2.5 py-0.5 text-[10px] font-black uppercase tracking-widest border-none shadow-sm text-white",
                    ...
                    <TableCell className="pr-8 text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="glass rounded-xl p-2 w-44 border-none">
                          <DropdownMenuItem className="rounded-lg font-bold py-2 text-sm">Edit Specs</DropdownMenuItem>
                          <DropdownMenuItem className="rounded-lg font-bold py-2 text-sm">Audit Log</DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem className="rounded-lg font-black text-sm text-destructive py-2">Terminate</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
                  </TableRow>
